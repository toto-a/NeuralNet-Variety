...
class BiFPNBLock(nn.Module):
    ...
    def forward(self, inputs_backbone : FakeBackBone ) :
        ...
        p4_up=self.p3_to_p4(weight[0] * p4  + weight[1] * p4_down + weight[2] * self.p3_up_to_4(p3_down))

        # Weights for p5, p5_first_node and p4_second_node to p5_second_node
        p5_w2=self.p5_w2_relu(self.p5_w2)
        weight= p5_w2 / (torch.sum(p5_w2, dim=0) + self.epsilon)
        p5_up=self.p4_to_p5(weight[0] * p5  + weight[1] * p5_down + weight[2] * self.p4_up_to_5(p4_up))

        # Weights for p6, p6_first_node and p5_second_node to p6_second_node
        p6_w2=self.p6_w2_relu(self.p6_w2)
        weight= p6_w2 / (torch.sum(p6_w2, dim=0) + self.epsilon)
        p6_up=self.p5_to_p6(weight[0] * p6  + weight[1] * p6_down + weight[2] * self.p5_up_to_6(p5_up))

        # Weights for p7, p6_second_node to p7_second_node
        p7_w2=self.p7_w2_relu(self.p7_w2)
        weight= p7_w2 / (torch.sum(p7_w2, dim=0) + self.epsilon)
        p7_up=self.p6_to_p7(weight[0] * p7  + weight[1] * self.p6_up_to_7(p6_up))
...
class BiFPN(nn.Module):
    ...
    def forward(self, inputs_backbone : FakeBackBone ) :
        p3, p4, p5, p6, p7 = inputs_backbone
        p3_out, p4_out, p5_out, p6_out, p7_out= self.n_bifpn(inputs_backbone)
...
if __name__=='__main__':
    ...
    model=BiFPN(64)
    out=model(FakeBackBone([256,512,1024],256)(inputs_backbone))
